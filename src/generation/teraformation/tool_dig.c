...
void change_iso_point_down(window_t *window, int x, int y, map_node_t **map2d)
{
	int new_z = 0;
...
void tool_dig_zone(window_t *window, int x, int y, map_node_t **map2d)
{
	...
	if (x + 2 > new_input.len_x || y + 2 > new_input.len_y)
		return;
	change_iso_point_down(window, x, y, map2d);
	change_iso_point_down(window, x + 1, y, map2d);
	change_iso_point_down(window, x, y + 1, map2d);
	change_iso_point_down(window, x + 1, y + 1, map2d);
...
void tool_dig(window_t *window, int x, int y, map_node_t **map2d)
{
	if (window->window_ui.tools_state.corner == sfFalse) {
		tool_dig_zone(window, x, y, map2d);
	} else {
		change_iso_point_down(window, x, y, map2d);
		window->stock_map2d = map2d;
		generate_texture(map2d);
